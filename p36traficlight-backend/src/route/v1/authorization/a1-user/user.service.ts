...
export default class UserService extends BaseService<UserDocument> {
  ...
  public async create(user: CreateUserDto): Promise<UserDocument> {
    ...
    if (phone) {
      ...
      if (userExists && !userExists.deleted) {
        responseMessage += 'Phone: already exists in the system; ';
      }
    }
    ...
    if (tokenLogin) {
      ...
      if (userExists && !userExists.deleted) {
        responseMessage += 'Token login: already exists in the system; ';
      }
    }
    ...
    if (email) {
      ...
      if (userExists && !userExists.deleted) {
        responseMessage += 'Email: already exists in the system; ';
      }
    }

    // if responseMessage not empty
  ...
  public async updateOneById(
    ...
    const userFindedById = await this.userRepository.findOneById(id);

    if (!userFindedById) throw new NotFoundException('User not found.');

    // Check phone exist
    if (phone) {
      const userExists = await this.userRepository.findByPhone(phone);
      if (userExists && userFindedById.phone !== phone && !userExists.deleted) {
        responseMessage += 'Phone: already exists in the system; ';
      }
    }
    ...
    if (tokenLogin) {
      ...
      if (userExists && userFindedById.tokenLogin !== tokenLogin && !userExists.deleted) {
        responseMessage += 'Token Login: already exists in the system; ';
      }
    }
    ...
    if (email) {
      ...
      if (userFindedById.email !== email && userExists && !userExists.deleted) {
        responseMessage += 'Email: already exists in the system; ';
      }
    }
