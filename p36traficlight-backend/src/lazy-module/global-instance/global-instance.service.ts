...
import CustomTCPService from '@lazy-module/tcp/tcp.service';
...
export default class GlobalInstanceService {
  constructor(
    ...
    private customTCPService: CustomTCPService,
  ) { }
   ...
   * @param host
   */
  tcpConnect(port: number, host: string) {
    this.customTCPService.connect(port, host,);
  }
   ...
   * @param data
   */
  tcpSendData(port: number, host: string, data: string) {
    this.customTCPService.emitData(port, host, data);
  }
   ...
   * @returns
   */
  getConnect(port: number, host: string) {
    return this.customTCPService.getConnect(port, host);
  }
  ...
  public addListenerHandleData(port: number, host: string) {
    this.customTCPService.addListenerHandleData(port, host);
  }
}
