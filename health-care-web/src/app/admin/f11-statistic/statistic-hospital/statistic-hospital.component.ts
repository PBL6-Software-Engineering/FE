...
@Component({
  selector: 'app-statistic-hospital',
  templateUrl: './statistic-hospital.component.html',
  styleUrls: ['./statistic-hospital.component.css'],
})
export class StatisticHospitalComponent implements OnInit {
  type = 'hospital';
  top = 5;
  dataDraw: any;
  data: any;
  ...
  ngOnInit(): void {
    this.statisticService.getStatisticByTop(this.top, this.type).subscribe({
      next: ({ data }) => {
        this.data = data.top_hospitals.data;
