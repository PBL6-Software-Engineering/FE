...
export class TypeDeviceService {
  ...
  get(): Observable<any> {
    return this.http
      .get<any>(this.apiURL + '/type-devices')
  ...
  delete(id: any) {
    return this.http
      .delete<any>(this.apiURL + '/type-devices/' + id, this.httpOptions)
      .pipe(retry(1), catchError(this.handleError));
  }
  ...
  deleteManyByIds(ids: any) {
    return this.http
      .delete<any>(
        `${this.apiURL}/type-devices/${ids}/ids`,
        this.httpOptions
      )
      .pipe(retry(1), catchError(this.handleError));
  }
  ...
  find(id: any): Observable<any> {
    return this.http
      .get<any>(this.apiURL + '/type-devices/' + id)
      .pipe(retry(1), catchError(this.handleError));
  }
  ...
  paginate(page: number, limit: number, filter: any): Observable<any> {
    let url =
      this.apiURL + '/type-devices/paginate?page=' + page + '&limit=' + limit;

    // add condition filter
    if (filter != '') {
      url =
        this.apiURL +
        '/type-devices/paginate?page=' +
        page +
        '&limit=' +
        limit +
        filter;
